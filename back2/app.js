...
const NORMALIZED_PUPIL_RADIUS = 34;
...
function updateCanvasLayout() {
  const shouldStack = window.innerWidth < 760;
  const nextWidth = shouldStack ? VIEW_WIDTH : VIEW_WIDTH * 2;
  const nextHeight = shouldStack ? VIEW_HEIGHT * 2 : VIEW_HEIGHT;
...
exportButton.addEventListener("click", () => {
  const payload = {
    ...
    assignmentNote: "기본 배정은 첫 번째 업로드 사진=우안, 두 번째 업로드 사진=좌안입니다.",
    eyes: {
      right: exportEye("right"),
      left: exportEye("left")
    }
  };
  ...
  link.href = url;
  link.download = `iris-mapping-pair-${Date.now()}.json`;
  link.click();
...
canvas.addEventListener("click", (event) => {
  ...
  // 두 번째 클릭 = 선 끝점
  const lineEndPoint = point;
  const pathMatches = resolveLinePath(lineStartPoint, lineEndPoint);
  ...
  eye.markers.push({
    ...
    x: Math.round(lineEndPoint.x),
    y: Math.round(lineEndPoint.y),
    codes: pathMatches.map((item) => item.code),
    organs: pathMatches.map((item) => item.organ),
    ...match
  });
  ...
  renderSelection({
    ...match,
    organ: `선 통과 위치: ${pathMatches.map((item) => item.code).join(" → ")}`
  });
...
function updateUiEnabled() {
  ...
  autoDetectButton.disabled = !enabled;
  clearMarkersButton.disabled = !enabled;
  exportButton.disabled = !state.eyes.right.image && !state.eyes.left.image;
...
function normalizePairView() {
  ...
  normalizeEyeView(state.eyes.right, NORMALIZED_IRIS_RADIUS, false);
  normalizeEyeView(state.eyes.left, NORMALIZED_IRIS_RADIUS, true);

  applyLeftEyeAutoOffset();
}

function normalizeEyeView(eye, targetRadius, keepPupilSize = false) {
  ...
  if (!keepPupilSize) {
    eye.pupilRadius = Math.max(22, Math.min(eye.pupilRadius * scale, targetRadius * 0.45));
  } else {
    eye.pupilRadius = NORMALIZED_PUPIL_RADIUS;
  }
...
function applyLeftEyeAutoOffset() {
  const eye = state.eyes.left;
  if (!eye.image) return;

  const zoomedWidth = eye.imageBounds.width * LEFT_EYE_AUTO_ZOOM;
  const zoomedHeight = eye.imageBounds.height * LEFT_EYE_AUTO_ZOOM;

  eye.imageBounds.x = eye.centerX - (eye.centerX - eye.imageBounds.x) * LEFT_EYE_AUTO_ZOOM;
  eye.imageBounds.y = eye.centerY - (eye.centerY - eye.imageBounds.y) * LEFT_EYE_AUTO_ZOOM;
  eye.imageBounds.width = zoomedWidth;
  eye.imageBounds.height = zoomedHeight;
  eye.imageBounds.scale *= LEFT_EYE_AUTO_ZOOM;

  alignLeftEyePupilToOverlay(eye);

  // 좌안 사진 강제 위치 보정
  eye.imageBounds.x += 70;
  eye.imageBounds.y += 150;
}

function alignLeftEyePupilToOverlay(eye) {
  drawImageOnly(eye);
  ...
  const samples = [];
  const left = bounds.x + bounds.width * 0.08;
  const right = bounds.x + bounds.width * 0.94;
  ...
  for (let y = top; y <= bottom; y += step) {
    for (let x = left; x <= right; x += step) {
      ...
      samples.push({ x, y, value, ...pixel });
    }
  }

  // 좌안은 홍채 외곽 원 중심을 먼저 찾기
let pupil = findIrisCenterByCircleEdge(imageData, bounds);

// 실패하면 기존 검은/푸른 동공 찾기
if (!pupil || isBadPupilResult(pupil, bounds)) {
  pupil = findBlueBlackPupilBlob(samples, bounds, step);
}

// 보정
if (pupil) {
  pupil = refineLeftPupilByDisc(imageData, pupil, bounds) || pupil;
}

  if (!pupil) return false;

  eye.imageBounds.x += NORMALIZED_CENTER_X - pupil.x;
  eye.imageBounds.y += NORMALIZED_CENTER_Y - pupil.y;

  return true;
}
function isBadPupilResult(pupil, bounds) {
  if (!pupil) return true;

  const minX = bounds.x + bounds.width * 0.25;
  const maxX = bounds.x + bounds.width * 0.75;
  const minY = bounds.y + bounds.height * 0.25;
  const maxY = bounds.y + bounds.height * 0.75;
...
function findIrisCenterByCircleEdge(imageData, bounds) {
  const cxMin = bounds.x + bounds.width * 0.35;
  const cxMax = bounds.x + bounds.width * 0.65;
  const cyMin = bounds.y + bounds.height * 0.35;
  const cyMax = bounds.y + bounds.height * 0.65;
  ...
  for (let cy = cyMin; cy <= cyMax; cy += 3) {
    for (let cx = cxMin; cx <= cxMax; cx += 3) {
      const score = irisCircleScore(imageData, cx, cy, bounds);
...
function irisCircleScore(imageData, cx, cy, bounds) {
  const minR = Math.min(bounds.width, bounds.height) * 0.18;
  ...
  for (let r = minR; r <= maxR; r += 2) {
    ...
    for (let a = 0; a < Math.PI * 2; a += Math.PI / 24) {
      ...
      const p1 = pixelAt(imageData, x1, y1);
      const p2 = pixelAt(imageData, x2, y2);

      if (!p1 || !p2) continue;

      const v1 = p1.r * 0.2126 + p1.g * 0.7152 + p1.b * 0.0722;
      const v2 = p2.r * 0.2126 + p2.g * 0.7152 + p2.b * 0.0722;

      score += Math.abs(v2 - v1);
      count++;
    }
...
function autoDetectIris(eye, eyeKey = state.activeEye) {
  ...
  const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
  const bounds = eye.imageBounds;
  const pupil = findPupilRegion(imageData, bounds, eyeKey);
  const refinedPupil = eyeKey === "left"
    ? refineLeftPupilByDisc(imageData, pupil, bounds) || refinePupilCircle(imageData, pupil, bounds)
    : refinePupilCircle(imageData, pupil, bounds);
  ...
  const estimatedOuter = candidates.length > 0 ? percentileNumber(candidates, 0.72) : null;
...
function drawIrisOverlay(eye, eyeKey = state.activeEye) {
  ...
  const pupil = eye.pupilRadius;
  const iris = eye.irisRadius;

  ctx.save();
  drawColoredZones(eye);
  ctx.lineWidth = 2;
...
function drawMarkers(eye = currentEye()) {
  ctx.save();
  for (const marker of eye.markers) {
    ctx.beginPath();
    ctx.arc(marker.x, marker.y, 7, 0, Math.PI * 2);
    ctx.fillStyle = "rgba(183,79,42,0.95)";
    ctx.fill();
    ctx.lineWidth = 2;
    ctx.strokeStyle = "#fff";
    ctx.stroke();
...
function resolvePoint(x, y) {
  ...
  const clockAngle = (angle + 90 + 360) % 360;
