...
function drawMarkers(eye = currentEye()) {
  ...
  eye.markers.forEach((marker, index) => {
    ...
    ctx.fillStyle = marker.id === state.selectedObservationId ? "rgba(215,174,74,0.98)" : "rgba(10,14,24,0.92)";
    ctx.fill();
    ctx.lineWidth = marker.id === state.selectedObservationId ? 3 : 2;
    ctx.strokeStyle = marker.id === state.selectedObservationId ? "#fff2b8" : "rgba(215,174,74,0.92)";
    ctx.stroke();
    ctx.fillStyle = "#ffffff";
    ctx.font = "900 13px Segoe UI, sans-serif";
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(toCircledNumber(index + 1), marker.x, marker.y);
  });
  ctx.restore();
}
